...
public class ExtandedSwitchExam {
	// 향상된 switch문 -> 람다식 결합
	// 메서드를 나누는 연습(main메서드 주 실행용, 나머지 메서드는(CRUD동작용))
	// main 메서드는 프로그램 실행 시 1순위로 동작
	...
	// 주메뉴
		...
		String kind = whoIsItReturn(Monster);	//아랫쪽에 만든 메서드를 호출(매개값)
	...
	static void whoIsIt(String Monster) { //호출 당함(매개값) -> 매개값은 타입만 맞으면 됨(즉, string)
		//void = 결과를 리턴하지 않는다.(자체적으로 해결)
