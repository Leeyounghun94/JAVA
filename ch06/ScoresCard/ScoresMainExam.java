...
public class ScoresMainExam {
	...
	public static void main(String[] args) {
		...
		while (run) { // 종료 run = false ;
			...
			System.out.println("1.학생관리 | 2.성적입력 | 3.통계 | 4.종료 | 5.메인메뉴로 돌아가기");
			System.out.println("=================================================");
			System.out.print(" > > >");
			int select = input.nextInt(); // 객체(scanner input) 사용 전에 private static 확인 필수
			switch (select) {
			case 1:
				System.out.println("학생관리 클래스로 진입");
				boolean strun = true;
				while (strun) {
					System.out.println("1.학생등록 | 2.학생보기 | 3.학생수정 | 4.학생삭제");
					System.out.print(" > > >");
			...
			default:
				...
				// break문 작성하면 꺼지기 때문에 작성을 안하는게 맞다.
	...
	private static Student find(String name) {
		...
		for( int i=0; i < st.length; i ++) {
			if(st[i] != null) {
				...
				if(arrayName.equals(name)) { // 배열name 과 키보드name  비교
